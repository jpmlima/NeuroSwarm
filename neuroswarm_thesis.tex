...
NeuroSwarm runs as a constellation of 17+ Unix processes managed by a parent process called the CerebralMatrix. Each process (``lobe'') is a standalone C++ executable that communicates exclusively through a ZeroMQ message bus. The CerebralMatrix fork-execs each lobe at startup, monitors them via \texttt{waitpid()}, and implements crash detection with exponential backoff (up to 5 restarts per lobe, with increasing cooldown periods).
...
\bottomrule
\end{tabular}
\caption{Core lobe inventory. Additional specialist lobes are generated at runtime.}
\end{table}
...
This cascade ensures the LLM is a last resort, not the default execution engine. As the operator registry grows, Tier 1 resolves an increasing fraction of goals without any LLM call.
...
\texttt{system\_info} & OS, hardware, and environment queries \\
\texttt{text\_processing} & grep, sed, awk, sort, etc. \\
\texttt{compilation} & Compiling source code \\
\texttt{script\_creation} & Generating and executing scripts \\
\texttt{memory\_analysis} & Inspecting system memory \\
\texttt{package\_management} & Querying installed packages \\
\texttt{user\_management} & User/group operations \\
\texttt{source\_modification} & Modifying source code files \\
\texttt{file\_search} & Finding files in the filesystem \\
\texttt{service\_management} & Querying system services \\
\bottomrule
\end{tabular}
\caption{The 14 capability domains tracked by the Basal Ganglia.}
\end{table}
