...
% AutoGPT: constant at 100% — very thick so it's clearly visible at the top
...
% Annotation — inside plot area, text box anchored to stay within bounds
\draw[<-, thick, gray] (axis cs:280, 8) -- (axis cs:200, 45) node[above, font=\footnotesize, text width=3.5cm, align=center] {Planner resolves most goals without inference};
...
\section{Limitations}
\begin{itemize}
    \item \textbf{Symbol grounding}: The system operates within the symbolic domain of a filesystem. No sensorimotor grounding --- no cameras, microphones, or actuators. A future \texttt{SensoryLobe} could integrate V4L2 (vision), ALSA/PulseAudio (audition), or GPIO (tactile), but this would substantially expand scope.
    \item \textbf{Single-machine bottleneck}: Inter-instance communication is limited to periodic SSH-based operator sync. The natural extension is ZMQ mesh networking --- each CerebralMatrix binds PUB/SUB to TCP peers rather than localhost --- but this requires distributed authentication and discovery.
    \item \textbf{Operator quality}: Postcondition inference is pattern-based and imperfect. LLM-assisted postcondition inference (``what facts changed?'') at the cost of one inference per novel operator is a viable improvement.
    \item \textbf{Genesis template rigidity}: The specialist template is a fixed C++ skeleton --- only domain-specific parameterisations, not novel architectures. Multiple template archetypes (observer, transformer, validator) or LLM-generated templates with compilation constraints would increase architectural diversity.
    \item \textbf{No formal verification}: As the system modifies itself, behaviour becomes irreducibly emergent. This is intrinsic to autopoietic systems: runtime invariant monitors (Homeostasis, Critic Lobe) provide empirical safety bounds but not formal guarantees. We consider this a fundamental property, not an engineering gap.
\end{itemize}
