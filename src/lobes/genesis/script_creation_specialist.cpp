...
// Auto-generated specialist lobe for domain: script_creation
// Created by BasalGanglia neurogenesis engine.

class ScriptCreationSpecialist {
public:
    ScriptCreationSpecialist(const std::string& pub_addr = "tcp://localhost:5555",
        ...
        std::cout << "[SCRIPT_CREATION_SPECIALIST] Specialist lobe online for domain: script_creation" << std::endl;
    }
...
private:
    ...
    static constexpr const char* CACHE_PATH = "./data/specialist_script_creation.json";

    // Domain keywords for filtering
    const std::vector<std::string> domain_keywords = {"#!/", "chmod +x", "bash -c", "sh -c"};
    ...
    void handle_request(const json& j) {
        ...
        json advice = {
            {"origin", "SCRIPT_CREATION_SPECIALIST"},
            {"intent", "specialist_advice"},
            {"domain", "script_creation"},
            {"command", cmd},
            {"pre_validations", json::array({"which bash || echo 'NO_BASH'", "test -w /tmp || echo 'TMP_NOT_WRITABLE'"})},
    ...
    void publish_report() {
        ...
        std::cout << "[SCRIPT_CREATION_SPECIALIST] Report: rate=" << (int)(rate * 100)
