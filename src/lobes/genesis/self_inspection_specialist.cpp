...
// Auto-generated specialist lobe for domain: self_inspection
// Created by BasalGanglia neurogenesis engine.

class SelfInspectionSpecialist {
public:
    SelfInspectionSpecialist(const std::string& pub_addr = "tcp://localhost:5555",
        ...
        std::cout << "[SELF_INSPECTION_SPECIALIST] Specialist lobe online for domain: self_inspection" << std::endl;
    }
...
private:
    ...
    static constexpr const char* CACHE_PATH = "./data/specialist_self_inspection.json";

    // Domain keywords for filtering
    const std::vector<std::string> domain_keywords = {"self_model", "neuroswarm", "du -s"};
    ...
    void handle_request(const json& j) {
        ...
        json advice = {
            {"origin", "SELF_INSPECTION_SPECIALIST"},
            {"intent", "specialist_advice"},
            {"domain", "self_inspection"},
            {"command", cmd},
            {"pre_validations", json::array({"test -f data/self_model.json || echo 'NO_SELF_MODEL'"})},
    ...
    void publish_report() {
        ...
        std::cout << "[SELF_INSPECTION_SPECIALIST] Report: rate=" << (int)(rate * 100)
