...
// Auto-generated specialist lobe for domain: bash_operation
// Created by BasalGanglia neurogenesis engine.

class BashOperationSpecialist {
public:
    BashOperationSpecialist(const std::string& pub_addr = "tcp://localhost:5555",
        ...
        std::cout << "[BASH_OPERATION_SPECIALIST] Specialist lobe online for domain: bash_operation" << std::endl;
    }
...
private:
    ...
    static constexpr const char* CACHE_PATH = "./data/specialist_bash_operation.json";

    // Domain keywords for filtering
    const std::vector<std::string> domain_keywords = {"bash_operation"};
    ...
    void handle_request(const json& j) {
        ...
        json advice = {
            {"origin", "BASH_OPERATION_SPECIALIST"},
            {"intent", "specialist_advice"},
            {"domain", "bash_operation"},
            {"command", cmd},
            {"pre_validations", json::array({})},
    ...
    void publish_report() {
        ...
        std::cout << "[BASH_OPERATION_SPECIALIST] Report: rate=" << (int)(rate * 100)
