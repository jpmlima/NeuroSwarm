...
// Auto-generated specialist lobe for domain: source_modification
// Created by BasalGanglia neurogenesis engine.

class SourceModificationSpecialist {
public:
    SourceModificationSpecialist(const std::string& pub_addr = "tcp://localhost:5555",
        ...
        std::cout << "[SOURCE_MODIFICATION_SPECIALIST] Specialist lobe online for domain: source_modification" << std::endl;
    }
...
private:
    ...
    static constexpr const char* CACHE_PATH = "./data/specialist_source_modification.json";

    // Domain keywords for filtering
    const std::vector<std::string> domain_keywords = {"sed ", "patch ", "diff ", "src/"};
    ...
    void handle_request(const json& j) {
        ...
        json advice = {
            {"origin", "SOURCE_MODIFICATION_SPECIALIST"},
            {"intent", "specialist_advice"},
            {"domain", "source_modification"},
            {"command", cmd},
            {"pre_validations", json::array({"test -d src/ || echo 'NO_SRC_DIR'", "test -w src/ || echo 'SRC_NOT_WRITABLE'"})},
    ...
    void publish_report() {
        ...
        std::cout << "[SOURCE_MODIFICATION_SPECIALIST] Report: rate=" << (int)(rate * 100)
...
int main() {
    SourceModificationSpecialist lobe;
    lobe.start();
    return 0;
}
