...
// Auto-generated specialist lobe for domain: file_write
// Created by BasalGanglia neurogenesis engine.

class FileWriteSpecialist {
public:
    FileWriteSpecialist(const std::string& pub_addr = "tcp://localhost:5555",
        ...
        std::cout << "[FILE_WRITE_SPECIALIST] Specialist lobe online for domain: file_write" << std::endl;
    }
...
private:
    ...
    static constexpr const char* CACHE_PATH = "./data/specialist_file_write.json";

    // Domain keywords for filtering
    const std::vector<std::string> domain_keywords = {"echo ", "tee ", "cp ", "mv ", "touch ", "mkdir ", ">>", "> "};
    ...
    void handle_request(const json& j) {
        ...
        json advice = {
            {"origin", "FILE_WRITE_SPECIALIST"},
            {"intent", "specialist_advice"},
            {"domain", "file_write"},
            {"command", cmd},
            {"pre_validations", json::array({"test -d "$(dirname '%CMD_TARGET%')"", "test -w "$(dirname '%CMD_TARGET%')" || echo 'DIR_NOT_WRITABLE'"})},
    ...
    void publish_report() {
        ...
        std::cout << "[FILE_WRITE_SPECIALIST] Report: rate=" << (int)(rate * 100)
